 ...
 * which Steam caches on disk. Scanning those cache files for the URLs yields
 * the (match_id, cluster_id, salt) triples the ingest endpoint needs.
 */

const REPLAY_URL = /http:\/\/replay(\d+)\.valve\.net\/1422450\/(\d+)_(\d+)\.(meta|dem)\.bz2/g
